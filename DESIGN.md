...
- **Segment switch / tabs:** pill or 8–12px buttons, `11px 20–22px` padding; unselected = white with hairline border and slate text, selected = solid AllBlue with white text. Used for product-category tabs.
- **Audience tabs (header, on blue):** the three audiences — Rodiny / Podnikatelé / Města a obce — are always visible in the header as tabs, never behind a dropdown button (user, 2026-08-18): the header has to show at a glance that the site has three versions. On blue the polarity flips — unselected is translucent white text, selected is either flush with the header plane (tabs recessed around it), a solid white plate, or a 3px white underline sitting on the header's bottom edge. Shape is still open; the three live header layouts are in `src/headerVariants.js` under `Hlavička` in the debug panel — two of them (`bila`, `modra`) follow Figma `Consolidation` node 1966:473: a strip of audience tabs above the bar that tucks away on scroll, drawn once on white and once on blue. In all of them the logo is the first thing top-left, and the whole catalogue sits under one `Produkty` item on the same level as the audience tabs — never above them, since audience is the first level of navigation and the catalogue is what you pick inside it. Labels are always the full wording — `Rodiny a jednotlivci`, never `Rodiny` — and never change with window width; the header buys the space by tightening padding instead. Picking an audience always navigates to the landing page, since that is where every audience's own version of the site lives.
